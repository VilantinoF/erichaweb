...
class Mesin extends BaseController
{
    ...
    public function index()
    {
        ...
        if ($session->get('role') == 3) {
            ...
            return view('user/dosen/mesin', $data);
        }
        $parsing = explode('/', uri_string());
        // dd($parsing);
        if (!empty($parsing[1]) and !empty($parsing[2]) and !empty($parsing[3])) {
        ...
        return view('user/admin/mesin', $data);
    }
    ...
    public function addFile()
    {
        ...
        if (!$file->isValid()) {
            ...
            $file->move('files/mesin', $newFileName);
            $data = [
        ...
        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                ...
                return redirect()->to('/mesin/' . $this->request->getVar('folderId') . '/' . $this->request->getVar('subFolderId') . '/' . $this->request->getVar('subSubFolderId'));
            } elseif ($this->request->getVar('subFolderId') != null) {
                session()->setFlashdata('pesan', '<div class="alert alert-success alert-dismissible fade show" role="alert">
        <strong>File berhasil ditambahkan</strong>
        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
        </div>');
                return redirect()->to('/mesin/' . $this->request->getVar('folderId') . '/' . $this->request->getVar('subFolderId'));
            } else {
                session()->setFlashdata('pesan', '<div class="alert alert-success alert-dismissible fade show" role="alert">
        <strong>File berhasil ditambahkan</strong>
        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
        </div>');
                return redirect()->to('/mesin/' . $this->request->getVar('folderId'));
            }
        }
    }
    ...
    public function deleteFile($id)
    {
        ...
        unlink('files/mesin/' . $file['store_file']);
    ...
    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
        ...
        if (!empty($parsing[4]) and !empty($parsing[5]) and !empty($parsing[6])) {
            ...
            return redirect()->to('mesin/' . $folderId . '/' . $subFolderId . '/' . $subSubFolderId);
        } elseif (!empty($parsing[4]) and !empty($parsing[5])) {
            $folderId = $parsing[4];
            $subFolderId = $parsing[5];
            return redirect()->to('mesin/' . $folderId . '/' . $subFolderId);
        } else {
            $folderId = $parsing[4];
            return redirect()->to('mesin/' . $folderId);
        }
    }
    ...
    public function downloadFile($id)
    {
        ...
        $file = $this->filesModel->find($id);
        // dd($file);
        return $this->response->download('files/mesin/' . $file['store_file'], null)->setFileName($file['file']);
    }
}
