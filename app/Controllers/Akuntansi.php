...
class Akuntansi extends BaseController
{
    ...
    public function addFile()
    {
        ...
        if (!$file->isValid()) {
            ...
            $file->move('files/akuntansi', $newFileName);
            $data = [
        ...
        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                ...
                return redirect()->to('/akuntansi/' . $this->request->getVar('folderId') . '/' . $this->request->getVar('subFolderId') . '/' . $this->request->getVar('subSubFolderId'));
            } elseif ($this->request->getVar('subFolderId') != null) {
                session()->setFlashdata('pesan', '<div class="alert alert-success alert-dismissible fade show" role="alert">
        <strong>File berhasil ditambahkan</strong>
        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
        </div>');
                return redirect()->to('/akuntansi/' . $this->request->getVar('folderId') . '/' . $this->request->getVar('subFolderId'));
            } else {
                session()->setFlashdata('pesan', '<div class="alert alert-success alert-dismissible fade show" role="alert">
        <strong>File berhasil ditambahkan</strong>
        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
        </div>');
                return redirect()->to('/akuntansi/' . $this->request->getVar('folderId'));
            }
        }
    }
    ...
    public function deleteFile($id)
    {
        ...
        unlink('files/akuntansi/' . $file['store_file']);
    ...
    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
        ...
        if (!empty($parsing[4]) and !empty($parsing[5]) and !empty($parsing[6])) {
            ...
            return redirect()->to('akuntansi/' . $folderId . '/' . $subFolderId . '/' . $subSubFolderId);
        } elseif (!empty($parsing[4]) and !empty($parsing[5])) {
            $folderId = $parsing[4];
            $subFolderId = $parsing[5];
            return redirect()->to('akuntansi/' . $folderId . '/' . $subFolderId);
        } else {
            $folderId = $parsing[4];
            return redirect()->to('akuntansi/' . $folderId);
        }
    }
    ...
    public function downloadFile($id)
    {
        $file = $this->filesModel->find($id);
        // dd($file);
        return $this->response->download('files/akuntansi/' . $file['store_file'], null)->setFileName($file['file']);
    }
}
